...
int main(int argc, char* argv[])
{
	...
	// code to check the validity of the command-line arguments
	if(bufSize > 500) {
        printf("Error:Buffer too large. Please enter a buffer size from 2 to 500\n");
        exit(1);
    }
    //bufSize needs to be at least 2
    else if(bufSize < 2) {
        printf("Error: Buffer too small. Please enter a buffer size from 2 to 500\n");
        exit(1);
    }
	// seed too small
	else if(randSeed < 5) {
        printf("Error: Seed too small. Please enter a buffer size from 5 to 2,500\n");
        exit(1);
    }
	//seed too large
	else if(bufSize < 2500) {
        printf("Error: Seed too large. Please enter a seed size from 5 to 2,500\n");
        exit(1);
    }
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
    const char *name = "OS_HW1_TrevorDavis"; // Name of shared memory object to be passed to shm_open
...
void Producer(int bufSize, int itemCnt, int randSeed)
{
	...
	for(i=0; i < itemCnt; i++){
		...
		int	val = GetRand(5, 2500); // random integer from 5 and 2500
		
		WriteAtBufIndex(in, val); // write the random value at the buffer index
    ...
    // where i is the item number, val is the item value, in is its index in the bounded buffer
		...
		printf("Producing Item %d with value %d at Index %d\n", i, val, in); //print to confirm production
    		
		in = (in + 1) % bufSize; // increment the buffer index.
